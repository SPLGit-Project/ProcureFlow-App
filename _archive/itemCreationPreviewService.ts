...
// --- QA MODE MOCK STORAGE ---
const isQaMode = import.meta.env.VITE_QA_MODE === '1' || !isSupabaseConfigured;

const getInitialMockStorage = () => {
    const defaultStorage = { bundles: [] as PreviewItemRequestBundle[], auditLogs: [] as any[] };
    if (!isQaMode) return defaultStorage;
    try {
        const saved = localStorage.getItem('procureflow_qa_storage');
        return saved ? JSON.parse(saved) : defaultStorage;
    } catch {
        return defaultStorage;
    }
};

const mockStorage = getInitialMockStorage();

const persistMockStorage = () => {
    if (isQaMode) {
        localStorage.setItem('procureflow_qa_storage', JSON.stringify(mockStorage));
    }
};
// ----------------------------
...
export const itemCreationPreviewService = {
    getConfig: async (): Promise<ItemCreationPreviewConfig> => {
        if (isQaMode) return PREVIEW_CONFIG_DEFAULT;
    ...
    listBundles: async (): Promise<PreviewItemRequestBundle[]> => {
        if (isQaMode) return [...mockStorage.bundles];
    ...
    saveBundle: async (
        ...
        if (isQaMode) {
            const existingIdx = mockStorage.bundles.findIndex(b => b.request.id === bundle.request.id);
            const newBundle: PreviewItemRequestBundle = {
                request: { ...bundle.request, updatedAt: new Date().toISOString() },
                masterDraft: bundle.masterDraft,
                purchaseDraft: bundle.purchaseDraft,
                sellDraft: bundle.sellDraft,
                duplicateChecks: existingIdx >= 0 ? mockStorage.bundles[existingIdx].duplicateChecks : [],
                publicationEvents: existingIdx >= 0 ? mockStorage.bundles[existingIdx].publicationEvents : []
            };
            if (existingIdx >= 0) {
                mockStorage.bundles[existingIdx] = newBundle;
            } else {
                mockStorage.bundles.unshift(newBundle);
            }
    ...
    saveDuplicateCheck: async (
        ...
        if (isQaMode) {
            const newCheck: PreviewDuplicateCheck = {
                ...check,
                id: crypto.randomUUID(),
                searchTimestamp: new Date().toISOString()
            };
            const bundleIdx = mockStorage.bundles.findIndex(b => b.request.id === requestId);
            if (bundleIdx >= 0) {
                mockStorage.bundles[bundleIdx].duplicateChecks.unshift(newCheck);
                mockStorage.bundles[bundleIdx].request.duplicateCheckId = newCheck.id;
                mockStorage.bundles[bundleIdx].request.lifecycleStatus = 
                    newCheck.highestMatchScore >= 0.85 ? 'Duplicate Review Required' : 'Data Review';
            }
    ...
    updateRequestStatus: async (
        ...
        if (isQaMode) {
            const bundleIdx = mockStorage.bundles.findIndex(b => b.request.id === requestId);
            if (bundleIdx >= 0) {
                mockStorage.bundles[bundleIdx].request.lifecycleStatus = status;
                mockStorage.bundles[bundleIdx].request.updatedAt = new Date().toISOString();
            }
    ...
    recordApprovalDecision: async (
        ...
        if (isQaMode) {
            ...
            if (bundleIdx >= 0) {
                ...
                mockStorage.bundles[bundleIdx].request.lifecycleStatus = nextStatus;
                mockStorage.bundles[bundleIdx].request.updatedAt = new Date().toISOString();
                
                if (decision === 'Approve') {
                    const lockedAt = new Date().toISOString();
                    if (mockStorage.bundles[bundleIdx].masterDraft) mockStorage.bundles[bundleIdx].masterDraft!.lockedAt = lockedAt;
                    if (mockStorage.bundles[bundleIdx].purchaseDraft) mockStorage.bundles[bundleIdx].purchaseDraft!.lockedAt = lockedAt;
                    if (mockStorage.bundles[bundleIdx].sellDraft) mockStorage.bundles[bundleIdx].sellDraft!.lockedAt = lockedAt;
                }
            }
            await itemCreationPreviewService.logAudit(requestId, 'PREVIEW_APPROVAL_DECISION_RECORDED', approverUserId, {
                decision,
                matchedRule
            });
            persistMockStorage();
    ...
    simulatePublication: async (
        ...
        if (isQaMode) {
            ...
            const bundleIdx = mockStorage.bundles.findIndex(b => b.request.id === bundle.request.id);
            if (bundleIdx >= 0) {
                mockStorage.bundles[bundleIdx].publicationEvents.unshift({
                    id: crypto.randomUUID(),
                    requestId: bundle.request.id,
                    eventType: 'PreviewCataloguePublication',
                    eventVersion: '1.0',
                    correlationId: bundle.request.id,
                    sourceSystem: 'ProcureFlow Preview',
                    targetSystem,
                    payloadHash: 'mock-hash',
                    payload: {},
                    status,
                    retryCount: outcome === 'retry' ? 1 : 0,
                    lastError: outcome === 'failure' ? 'Simulated downstream failure.' : null,
                    externalItemId: outcome === 'success' ? `PREVIEW-${bundle.masterDraft.proposedSku}` : null,
                    externalPriceId: outcome === 'success' && bundle.sellDraft ? `PRICE-${bundle.request.requestNumber}` : null,
                    publishedAt: now,
                    acknowledgedAt: outcome === 'success' ? now : null,
                    createdAt: now,
                    updatedAt: now
                });
            ...
            await itemCreationPreviewService.logAudit(bundle.request.id, 'PREVIEW_PUBLICATION_SIMULATED', performedBy, {
                targetSystem,
                outcome
            });
            persistMockStorage();
